...
`/opt/webterm/.env`, and a redeploy.

**With a Cloudflare token** (DNS-01) Traefik issues one wildcard certificate covering
every slug at once. **Without one** (HTTP-01, the default) forwards get no certificate at all: the router
