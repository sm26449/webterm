...
tmux set -p allow-passthrough on \
  || tmux set -g allow-passthrough on          # fallback for tmux < 3.3
printf '\033Ptmux;\033\033]133;%s\007\033\\' "$1"   # wrap for tmux
```

`-p` scopes passthrough to this pane. The `-g` fallback turns it on server-wide, which
is what the note further down warns about — it exists only so shell integration works
at all on older tmux.
...
- **Install-time integrity**: when you enable it from the UI, the activation command
  arrives over the authenticated channel (browser ↔ gateway, TLS) and carries the
  script's **sha256**; the download verifies it before writing and sourcing it. On that
  path a MITM on the fetch cannot substitute the script, not even with
  `WEBTERM_AGENT_INSECURE=1` (unverified TLS), because the hash did not travel over the
  fetch.

  **The enrolment path is weaker and it is worth knowing why.** There, the hash is inside
  the install script you download with `curl -fsSk`. With `WEBTERM_AGENT_INSECURE=1` a MITM
  can replace the script and the hash in one move, since both come down the same unverified
  connection. That is not a flaw in the check — it is what `insecure` means. Enrol over a
  domain with real TLS, then the UI path applies for everything after.
