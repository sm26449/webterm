...
trusted with the whole fleet** — like the people who share root on a jump host.
...
```
[ Browser ]  --TLS-->  [ Gateway ]  --TLS-->  [ Agent (root on host) ]  -->  [ Host: PTY, files, commands, forwards ]
...
- **Agent ↔ Host**: the agent runs with its user's privileges — usually **root**.
...
- **Post-incident reconstruction**: every state-changing API request is written to
  ...
  cannot silently escape it; request bodies are never stored. With one account it
...
1. **The agent runs with high privileges (often root).** Anyone who gets past
   login has access to the agent user's shell and files on **all** hosts —
   exactly like SSH. There is no fs/command sandbox. *Recommended mitigation:*
   install the agent as a **dedicated, least-privilege user** where you can.
...
3. **The gateway is a single point of total compromise.** It commands the agents
   as root. A compromised gateway (RCE) = a compromised fleet. The update
...
Preferences. With it off, the gateway initiates no outbound connection you did not
configure. The other outbound paths exist only because you set them up: SMTP alerts,
...
2. **Agent as a least-privilege user**, not root, where you can.
