...
import { api, ApiError, CommandGuard, Host } from '../lib/api'
...
export default function FleetRunModal(props: { hosts: Host[]; onClose: () => void }) {
  ...
  async function run() {
    ...
    await Promise.all(chosen.map(async (h) => {
      try {
        ...
        setResults((prev) => ({ ...prev, [h.id]: { status: 'error', error: e instanceof ApiError ? e.message : t('fleet.error') } }))
      }
    }))
  }
  ...
  async function copyReport() {
    await navigator.clipboard.writeText(reportMarkdown()).catch(() => {})
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={t('nav.fleetRunAria')}
                  ...
                  {runnable.map((h) => (
                    <button key={h.id} onClick={() => toggle(h.id)}
                      className={`rounded-lg border px-2.5 py-1 font-mono text-[13px] ${
                        selected.has(h.id) ? 'border-sky-500 bg-sky-500/10 text-sky-300' : 'border-ink-700 bg-ink-800 text-slate-400 hover:border-ink-600'}`}>
        ...
        {phase === 'confirm' && (
          <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-4">
            <div className="flex items-center gap-2 font-medium text-amber-300">⚠ {t('fleet.youRunOn')} {chosen.length} {chosen.length === 1 ? t('fleet.host') : t('fleet.hosts')}</div>
          ...
          {phase === 'confirm' && (
            <>
              <button onClick={run}
                ...
                {t('fleet.runOn')} {chosen.length} {chosen.length === 1 ? t('fleet.host') : t('fleet.hosts')}
              </button>
