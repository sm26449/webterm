...
import { api, getBootVersion, Host, SearchHit, Session } from '../lib/api'
...
export default function Sidebar(props: {
  ...
  async function deleteHost(host: Host) {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : t('sidebar.error'))
    }
  }

  async function uninstallHost(host: Host) {
    if (!confirm(`Uninstall the agent from “${host.name}”?\n\nThe agent stops and removes its`
      + ` files (~/.webterm), its supervision (systemd/cron) and the tmux sessions on the server,`
      + ` then it is removed from WebTerm. Irreversible — it must be re-enrolled to come back.`)) return
    try {
      ...
      const m = e instanceof Error ? e.message : t('sidebar.error')
      // agent offline / nu răspunde → oferă scoaterea DOAR din WebTerm (fișierele rămân)
      if (/offline|did not answer|not responding/i.test(m)
        ...
        try {
          ...
          props.onChanged()
        } catch (e2) { alert(e2 instanceof Error ? e2.message : t('sidebar.error')) }
      } else if (!/offline|did not answer|not responding/i.test(m)) {
        alert(m)
      }
    }
  }
  ...
  async function moveToFolder(host: Host) {
    ...
    }).catch((e) => alert(e instanceof Error ? e.message : t('sidebar.error')))
    props.onChanged()
  }
  ...
  async function provision(host: Host) {
    ...
    try {
      ...
      alert(t('sidebar.provisionFailed') + (e instanceof Error ? e.message : t('sidebar.error')))
    } finally {
  ...
  const renderHost = (host: Host) => {
    const liveCount = props.sessions.filter(
      (s) => s.host_id === host.id && (s.state === 'live' || s.state === 'creating')).length
      ...
      <div key={host.id} className="px-2 py-0.5">
        ...
        >
          ...
          <button
            ...
            title={host.connection_type !== 'agent' ? t('sidebar.connect') : host.online ? t('sidebar.newSession') : 'host offline'}
            aria-label={t('host.newSession')}
  ...
  const body = (
    <div className="wt-sidebar flex h-full w-72 flex-col border-r border-ink-800 bg-ink-900">
      ...
      <div className="relative px-3 py-2">
        <span className="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 text-slate-600">
          <SearchIcon />
        </span>
        <input
          data-wt-search
      ...
      <div className="flex-1 overflow-y-auto">
                ...
                {showHeader && (
                  <div className="group/folder flex items-center gap-1 px-3 py-1.5 text-xs font-medium uppercase tracking-wide text-slate-500">
                    ...
                    >
                      ...
                      <span className={`truncate ${folder ? '' : 'italic text-slate-600'}`}>{folder || t('sidebar.noFolder')}</span>
                    </button>
                    {folder && (
                      <button
                    ...
                    <span className="shrink-0 text-slate-600">{inFolder.length}</span>
                  </div>
                )}
        ...
        {historyHits !== null && (
          <div className="border-t border-ink-800 pb-2">
            <div className="px-4 pb-1 pt-3 text-xs font-medium uppercase tracking-wide text-slate-600">
              In session history
            </div>
            {historyHits.length === 0 && (
              <div className="px-4 py-1 text-xs text-slate-600">{t('sidebar.noResults')}</div>
            )}
            {historyHits.map((h) => (
              <button
                key={h.id}
      ...
      >
        ...
        <span className={`font-mono text-slate-600 ${newVersion ? '' : 'ml-auto'}`}>v{version || '—'}</span>
      </button>
    </div>
  )
