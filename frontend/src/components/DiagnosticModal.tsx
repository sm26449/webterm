...
const META: Record<string, { icon: string; cls: string; label: string }> = {
  connect: { icon: '🟢', cls: 'text-emerald-300', label: 'diag.evtConnect' },
  disconnect: { icon: '🔴', cls: 'text-rose-300', label: 'diag.evtDisconnect' },
  ...
  update_applied: { icon: '✅', cls: 'text-emerald-300', label: 'diag.evtUpdateApplied' },
...
export default function DiagnosticModal(props: { host: Host; onClose: () => void }) {
  ...
  const loadLog = () => {
    ...
    api<{ log: string }>(`/api/hosts/${props.host.id}/agent-log`)
      ...
      .catch((e) => setLogErr(e instanceof ApiError ? e.message : t('diag.logLoadFailed')))
  ...
  const load = () => {
    ...
    api<Diag>(`/api/hosts/${props.host.id}/events`)
      .then(setDiag)
      .catch((e) => setErr(e instanceof ApiError ? e.message : t('diag.eventsLoadFailed')))
        ...
        {diag && (
          <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
            <div className="rounded-lg bg-ink-800 px-3 py-2">
              ...
              <div className={diag.online ? 'text-emerald-300' : 'text-rose-300'}>
