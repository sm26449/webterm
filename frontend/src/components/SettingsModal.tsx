...
import { api, ApiError, CommandGuard, WatermarkConfig } from '../lib/api'
...
export default function SettingsModal(props: {
  ...
  const saveGuard = async () => {
    try {
      ...
      setGuardMsg(e instanceof ApiError ? e.message : t('settings.saveError'))
    }
  }
  ...
  async function addToken(e: FormEvent) {
    ...
    try {
      ...
      setTokErr(e instanceof ApiError ? e.message : String(e))
    }
    setBusy(false)
  }
  ...
  async function addUser(e: FormEvent) {
    ...
    try {
      ...
      setUsersErr(e instanceof ApiError ? e.message : String(e))
    }
    setBusy(false)
  }
  ...
  const copyRedirect = () => {
    if (!cloud) return
    navigator.clipboard.writeText(cloud.redirect_uri).then(() => {
      setCopied(true)
  ...
  async function saveThresholds() {
    ...
    try {
      ...
      setSmtpErr(err instanceof ApiError ? err.message : t('settings.error'))
  ...
  const importTheme = async (file: File) => {
    ...
    try {
      ...
      setImportErr(e instanceof Error ? e.message : t('settings.importFailed'))
    }
  }
  ...
  async function saveFwd() {
    ...
    try {
      ...
      setFwdErr(e instanceof ApiError ? e.message : t('settings.error'))
  ...
  async function downloadBackupNow() {
    ...
    try {
      ...
      setBkErr(e instanceof ApiError ? e.message : t('settings.backup.failed'))
  ...
  async function downloadStored(name: string) {
    ...
    try {
      ...
      setBkErr(e instanceof ApiError ? e.message : t('settings.downloadFailed'))
    } finally { setBkBusy(false) }
  }
  ...
  async function deleteStored(name: string) {
    ...
    try {
      ...
      await loadBackup()
    } catch (e) { setBkErr(e instanceof ApiError ? e.message : t('settings.deleteFailed')) }
  }
  ...
  async function saveSchedule(schedule: 'off' | 'daily' | 'weekly') {
    ...
    } catch (e) { setBkErr(e instanceof ApiError ? e.message : t('settings.error')) }
  }
  ...
  async function doRestore() {
    ...
    try {
      ...
      setBkErr(e instanceof ApiError ? e.message : t('settings.backup.restoreFailed'))
    } finally { setBkBusy(false) }
  }
  ...
  async function importSigningKey() {
    ...
    } catch (e) { setSignErr(e instanceof ApiError ? e.message : t('settings.importFailed')) } finally { setSignBusy(false) }
  }
  ...
  async function genSigningKey() {
    ...
    try {
      ...
      props.onAccountChanged()
    } catch (e) { setSignErr(e instanceof ApiError ? e.message : t('settings.sign.genFailed')) } finally { setSignBusy(false) }
  }
  async function unlockSigning() {
    ...
    try {
      ...
      props.onAccountChanged()
    } catch (e) { setSignErr(e instanceof ApiError ? e.message : t('settings.sign.unlockFailed')) } finally { setSignBusy(false) }
  }
  async function lockSigning() {
    setSignErr(''); setSignMsg('')
    try { const s = await api<SignStatus>('/api/signing/lock', { method: 'POST' }); setSign(s); props.onAccountChanged() }
    catch (e) { setSignErr(e instanceof ApiError ? e.message : t('settings.error')) }
  }
  async function downloadSigningKey() {
    ...
    } catch (e) { setSignErr(e instanceof ApiError ? e.message : t('settings.downloadFailed')) } finally { setSignBusy(false) }
  }
  ...
  async function startEnroll() {
    ...
    try {
      ...
      setSecurityErr(err instanceof ApiError ? err.message : t('settings.error'))
    }
  }
  ...
  async function saveAccount(e: FormEvent) {
    ...
    try {
      ...
      setAccountErr(err instanceof ApiError ? err.message : t('settings.error'))
    } finally {
      setBusy(false)
    }
  }
  ...
  async function remove(id: number) {
    ...
    try {
      ...
      setSecurityErr(err instanceof ApiError ? err.message : t('settings.deleteFailed'))
    }
    load()
  }
        ...
        <ul className="mt-2 flex flex-col gap-1">
          {users.map((u) => (
            <li key={u.id} className="flex items-center gap-2 rounded-lg bg-ink-800/60 px-3 py-2 text-sm ring-1 ring-ink-700">
              ...
              {u.is_self && <span className="shrink-0 rounded bg-sky-600/20 px-1.5 py-0.5 text-[11px] text-sky-300">{t('settings.users.you')}</span>}
          ...
          {(audit ?? []).map((e, i) => (
            <li key={`${e.ts}-${i}`}
              ...
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                ...
                <span className={`font-mono ${e.status >= 400 ? 'text-rose-400' : 'text-emerald-400'}`}>
                  {e.status}
                </span>
