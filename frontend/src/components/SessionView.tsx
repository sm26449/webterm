...
import { api, ApiError, CommandGuard, Host, Session, withStepup } from '../lib/api'
...
// scrollback adaptiv: 10k linii ≈ 10-15 MB per terminal cu buffer plin — pe
// device-uri cu RAM puțin reducem (deviceMemory lipsește pe Safari/FF → 10k)
const SCROLLBACK = (navigator as { deviceMemory?: number }).deviceMemory! <= 4 ? 3000 : 10000
...
export default function SessionView(props: {
  ...
  async function createShareLink() {
    try {
      ...
      alert(e instanceof ApiError ? e.message : t('session.shareLinkFailed'))
    }
  }
  ...
  async function reconnectTelnet() {
    ...
    try {
      ...
      const m = e instanceof ApiError ? e.message : t('session.reconnectFailed')
  ...
  async function reauth() {
    ...
    try {
      ...
      setUnlocking(false)
      setLockErr(e instanceof ApiError ? e.message : t('session.passkeyAuthFailed'))
    }
  }
  ...
  const copyOutput = async (c: Command) => {
    ...
    await navigator.clipboard.writeText(text.slice(0, 200_000)).catch(() => {})
    setCopied(true)
    setTimeout(() => setCopied(false), 1200)
  }
  ...
  async function deleteSession() {
    ...
    try {
      ...
      alert(e instanceof ApiError ? e.message : t('session.deleteFailed'))
    }
  }
      ...
      {shareUrl && (
        <div className="flex items-center gap-2 border-b border-ink-800 bg-ink-900/70 px-3 py-2 text-sm">
          <span className={`shrink-0 rounded px-1.5 py-0.5 text-[11px] ${shareIsWritable ? 'bg-amber-500/15 text-amber-300' : 'bg-ink-800 text-slate-400'}`}>
            {shareIsWritable ? 'writable' : 'read-only'}
          </span>
          <code className="min-w-0 flex-1 truncate rounded bg-black/40 px-2 py-1 font-mono text-xs text-emerald-400">{shareUrl}</code>
          <button
            onClick={() => { navigator.clipboard.writeText(shareUrl); setCopied(true); setTimeout(() => setCopied(false), 1200) }}
