...
export default function FilePanel(props: { host: Host; sessionId: string; onClose: () => void; overlay?: boolean }) {
  ...
  const load = useCallback(async (p: string) => {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : t('files.genericErr'))
    }
  }, [props.host.id, t])
  ...
  async function doMkdir(name: string) {
    ...
    try {
      ...
      load(listing.path)
    } catch (e) { setError(e instanceof ApiError ? e.message : t('files.genericErr')) }
  }
  ...
  async function doRename(e: Entry, name: string) {
    ...
    try {
      ...
      load(listing.path)
    } catch (err) { setError(err instanceof ApiError ? err.message : t('files.genericErr')) }
  }
