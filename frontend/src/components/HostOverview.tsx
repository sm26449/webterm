...
import { api, Host, Session, timeAgo } from '../lib/api'
...
export default function HostOverview(props: {
  ...
  const active = allSessions.filter((s) => s.state === 'live' || s.state === 'creating')
  ...
  const selLive = sel ? sel.state === 'live' || sel.state === 'creating' : false
