...
import { api, AppState, Host, Session, Snippet, setStepupHandler } from './lib/api'
...
function MainApp() {
  ...
  useEffect(() => {
    ...
    if (s) {
      ...
      setSrAnnounce(t('app.srSessionActive', {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (openTabs[idx]) {
        e.preventDefault()
        window.location.hash = `/s/${openTabs[idx]}`
      }
    }
    window.addEventListener('keydown', onKey, true)
    return () => window.removeEventListener('keydown', onKey, true)
  ...
  // pe device-uri cu RAM puțin, cache-ul de terminale se strânge (fiecare
  // terminal cu scrollback plin ≈ 10-15 MB); deviceMemory lipsește pe
  // Safari/Firefox → rămâne 6 (desktop-ul duce)
  const KEEP_ALIVE = (navigator as { deviceMemory?: number }).deviceMemory! <= 4 ? 2 : 6
  ...
  if (secondSid && !second) {
    // the split session vanished (deleted); drop the split
    setSecondSid(null)
  }
  ...
  async function connectHost(host: Host) {
    ...
    try {
      ...
      notify(t('app.cannotStartSession'), e instanceof Error ? e.message : t('app.error'), 'warn')
    }
  }
  ...
  async function openSerial(host: Host, device: string, params: SerialParams): Promise<boolean> {
    ...
    try {
      ...
      notify(t('app.cannotOpenSerial'), e instanceof Error ? e.message : t('app.error'), 'warn')
      return false
    }
  }
