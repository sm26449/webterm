...
command -v apt-get >/dev/null || { err "Installer-ul cere Ubuntu/Debian (apt)."; exit 1; }
...
if [ "$FILES_SRC" != "$INSTALL_DIR" ]; then
  ...
  install -m 0755 "$FILES_SRC/deploy.sh" "$FILES_SRC/rollback.sh" "$INSTALL_DIR/"
...
ask CF_TOKEN "Token Cloudflare Zone:DNS:Edit" secret
...
PUBIP=$(getent hosts "$DOMAIN" | awk '{print $1; exit}')
