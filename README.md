...
> [!WARNING]
> **Read this before exposing it to a network.** WebTerm runs an agent on every host, often as
> root. **Anyone who gets past the login has root-equivalent access to your whole fleet** — the
> same as handing over an SSH key. It is built for a **single trusted administrator**: there are no
> roles, and the **gateway is a single point of total compromise**.
>
> So: use a **domain with HTTPS and passkeys**, not an IP with a password. Install agents as a
> **least-privilege user** where you can. What it defends against and what it does not:
...
Then put the port in `WEBTERM_PUBLIC_URL` **after** running `setup.sh` — edit `.env`
to `https://host:8443` and re-run `docker compose up -d`. `setup.sh` writes that
variable from the argument you give it and does not accept a port, and the agent
install command shown in the UI is generated from it, so it comes out wrong if the
two disagree.
...
```sh
# interactive (asks for domain, email, Cloudflare token):
git clone https://github.com/sm26449/webterm && cd webterm
sudo ./install.sh
...
sudo ./install.sh --non-interactive \
  ...
  --cf-token <cloudflare-token> --ghcr-token <github-token>
```
...
#   CF_DNS_API_TOKEN    = Cloudflare token (Zone:DNS:Edit on your zone)
...
**Upgrading: one command.** `cd /opt/webterm && ./upgrade.sh` (or `make upgrade`) takes the
...
| `CF_DNS_API_TOKEN` | Cloudflare token (Zone:DNS:Edit) for the DNS-01 certificate |
...
dangerous commands at Enter (e.g. `rm -rf`, `mkfs`) — client-side, editable.
...
file, share writable or not). Request bodies are never stored — passwords and file
contents don't reach the log. Retention via `WEBTERM_AUDIT_DAYS` (default 120 days).
...
`transcripts/`, `secret`. `make backup` writes a `.tar.gz` to `./backups`.
