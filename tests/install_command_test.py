...
async def main():
    ...
    check("varianta simplă e un one-liner curl|sh", plain.startswith("curl ") and plain.endswith("| sh"))
    ...
    check("o singură descărcare (un singur curl)", ded.count("curl ") == 1, ded)
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://t") as c:
        ...
        check("crearea hostului întoarce ambele variante",
              h.get("install_command", "").startswith("curl")
