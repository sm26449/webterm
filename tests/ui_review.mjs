/* Captură completă a suprafeței UI + audit de accesibilitate (axe-core).
   Rulează după boot_stack. Screenshot-uri în /tmp/webterm-review. */
...
try {
  ...
  for (const theme of ['macos', 'dark']) {
    ...
    if (theme === 'macos') await scan(page, 'login')

    await login(page)
    await page.screenshot({ path: `${OUT}/${theme}-02-empty.png` })
    if (theme === 'macos') await scan(page, 'empty state')
    ...
    if (theme === 'macos') await scan(page, 'session view')
    ...
    if (theme === 'macos') await scan(page, 'settings modal')
    ...
    if (theme === 'macos') await scan(page, 'file browser')
    ...
    await step('adaugă host', async () => {
      ...
      if (theme === 'macos') await scan(page, 'add host modal')
