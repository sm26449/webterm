...
- **The agent runs with high privileges** (often root) on every host; fs/command operations are NOT
  sandboxed. Anyone who gets past login has equivalent access across the whole fleet — like SSH.
- **There is no per-object authorization / RBAC** — the model is single-account.
