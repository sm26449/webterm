...
class SessionHub:
    ...
    def _maybe_cap(self) -> None:
        ...
        try:
            ...
            return
        if out_sz < config.TRANSCRIPT_MAX_BYTES:
            return
...
# (ar fi head-of-line blocking pe tot agentul). Fără plafon, un consumator lent +
    ...
    def _feed(self, data: bytes) -> None:      # din recv loop
        ...
        if self._buffered + len(data) > FWD_MAX_BUFFER:
            ...
            log.warning("forward %s: %d bytes in flight over the cap (%d) — tearing down "
                        "(consumator lent)", self.stream_id,
                ...
                try:
                    ...
                    try:
                        await record_agent_event(self.host_id, "reconcile_failed",
                                                 detail="raport de sesiuni invalid de la agent")
                    except Exception:         # noqa: BLE001
                        pass
                    if msg.get("event") == "hello":
                        try:
...
async def force_update_agent(host_id: int) -> dict:
    ...
    if not resp.get("ok"):
        ...
        raise RuntimeError("agentul a respins update-ul (%s)%s" % (code, " — " + hint if hint else ""))
